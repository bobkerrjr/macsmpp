...
#ifdef DEBUG
	cout << "SubmitMulti::SubmitMulti()" << endl;
#endif
	this->init();
}

SubmitMulti::SubmitMulti(char* buffer, uint32_t commandLength) {
#ifdef DEBUG
	cout << "SubmitMulti::SubmitMulti(char* buffer, uint32_t commandLength)" << endl;
#endif
	this->init();
...
void SubmitMulti::pduDecode(char * buffer, uint32_t commandLength) {
	int i=0;
	int j=0;
	...
	//Copy esm_class, protocol_id and protocol_id values
